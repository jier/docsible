        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Try):
                ...
                for handler in node.handlers:
                    handler_body = (
                        ast.unparse(ast.Module(body=handler.body, type_ignores=[]))
                        if hasattr(ast, "unparse")
                        else str(handler.body)
                    )

                    # Check if using print, click.echo, or logger
                    has_logging = any(
                        keyword in handler_body
                        for keyword in ["logger.", "logging.", "click.echo", "click.secho"]
                    )

                    # Check if silently passing
                    is_silent_pass = len(handler.body) == 1 and isinstance(
