...
class DocumentationValidator:
    ...
    def _validate_clarity(self, documentation: str) -> Tuple[List[ValidationIssue], Dict[str, Any]]:
        """
        ...
        required_sections = ['Description', 'Requirements', 'Variables', 'Dependencies', 'Example']
        found_sections = []
    ...
    def _validate_maintenance(
        ...
        if total_vars > 0:
            ...
            if not re.search(r'^#+\s+(?:Role\s+)?Variables', documentation, re.MULTILINE | re.IGNORECASE):
                issues.append(ValidationIssue(
    ...
    def _validate_truth(
        ...
        if complexity_report:
            ...
            category_mentions = re.findall(
                r'\b(SIMPLE|MEDIUM|COMPLEX)\s+role',
                documentation,
                re.IGNORECASE
            )
    ...
    def _validate_value(
        ...
        if complexity_report and complexity_report.category.value in ['medium', 'complex']:
            if not has_diagrams:
                issues.append(ValidationIssue(
        ...
        has_security = bool(re.search(r'security|credentials?|authentication', documentation, re.IGNORECASE))
